    ...
    companion object {
        ...
        fun menuAdministrarTerapeutas() {
            ...
            while (seguir) {
                ...
                when (option) {
                    1 -> aniadirTerapeuta()
                    2 -> asignarGrupoTerapeuta()
                    3 -> asignarMetodoTerapeuta()
                    4 -> eliminarTerapeuta()
                    5 -> mostrarTerapeutas()
...
//FUNCIONES DE MENU TERAPEUTA -- PARTE FAMILIAS
        ...
        private fun modificarFamilia() {
            ...
            println("Inserte id de familia a modificar: ")
            var id = readln().toInt()
            println("Inserte nueva descripcion: ")
            var descripcion = readlnOrNull() ?: ""
            familia.id = id
            familia.descripcion = descripcion
            if (ControladorFamilia.actualizarFamilia(familia)) {
...
//    val metodologia = readlnOrNull() ?: ""
         ...
         */
        private fun asignarGrupoTerapeuta() {
            print("Ingrese el email del terapeuta para asignarle un grupo: ")
            val email = readlnOrNull() ?: ""
            val terapeuta = ControladorTerapeuta.obtenerTerapeuraPorEmail(email)
            var index = false
            if (terapeuta != null) {
                do {
                    index = mostrarGrupos(index, email, terapeuta)
                } while (!index)
            }
        }

        private fun mostrarGrupos(index: Boolean, email: String, terapeuta: Terapeuta): Boolean {
            var index1 = index
            ...
            do {
                ...
                print("Asigne uno de los siguientes grupos al terapeuta: ")
                var grupo = readln().toInt()
                for (i in a) {
                    if (grupo == i.id) {
                        terapeuta.grupo = grupo
                        index1 = true
                        ControladorTerapeuta.cambiarGrupo(email, grupo)
                        println("Grupo asignado correctamente")
                    }
                }
            } while (!index1)
            println("Elige un terapeuta que exista")
            return index1
        }
         ...
         */
        private fun asignarMetodoTerapeuta() {
            print("Ingrese el email del terapeuta al que le va a asignar metodología: ")
            val email = readlnOrNull() ?: ""
            val terapeuta = ControladorTerapeuta.obtenerTerapeuraPorEmail(email)
            var metodologia = ""
            if (terapeuta != null) {
